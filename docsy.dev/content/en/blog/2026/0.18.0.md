...
- **If you produce MarkMap markup other than through a `markmap` fence on the
  ...
  longer sees it. Set the `hasMarkmap` page flag from a `hooks/head-end.html`
  partial; for the affected authoring paths and the one-line partial, see [When
  a mind map doesn't render][markmap-not-rendering].
...
[markmap-not-rendering]:
  /docs/content/diagrams-and-formulae/#when-a-mind-map-doesnt-render
