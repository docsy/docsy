...
- **If you set `params.markmap.enable`, `params.markmap.version`, or
  `params.disable_click2copy_chroma`**: all three are deprecated and keep
  working for this release cycle, with a build warning (a present but empty
  `params.markmap.version` fails the build). Migrate to the registry, then
  remove the old parameter: for MarkMap, the [MarkMap guide][markmap-guide]
  (which also says why the old `enable` must go); for the copy button,
  `click-to-copy: false` under [`params.docsy.plugins`][ug-plugins].
