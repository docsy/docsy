...
- **If you set `params.mermaid.version`**: the pin moved to the `mermaid`
  registry entry. If you had overridden the theme's pin, set `version` on the
  entry, then remove the legacy parameter; if not, just remove it. The legacy
  value is honored for this release cycle, with a build warning. Pins below
  Mermaid 10 no longer render, and range pins are no longer accepted; the other
  `params.mermaid` settings are unchanged. See [Mermaid
