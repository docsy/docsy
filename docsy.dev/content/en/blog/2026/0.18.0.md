...
- **If you set `params.markmap.enable`, `params.markmap.version`, or
  `params.disable_click2copy_chroma`**: these parameters are deprecated. Migrate
  to the registry, then remove the old parameters. For MarkMap's migration and
  legacy behavior, see the [MarkMap guide][markmap-guide]. For the copy button,
  set `click-to-copy: false` under [`params.docsy.plugins`][ug-plugins].
