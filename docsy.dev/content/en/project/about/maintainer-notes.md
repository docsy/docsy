...
is enough. Guarded by the [script-version-pins test](#test-suites). Before
bumping, check the [npm registry][npm-registry] and [OSV][] for advisories
affecting the target version. Verify that a page using the dependency (the
diagrams and formulae page, for example) renders with the new pin. Renovate
proposes routine bumps (see [Dependency updates](#dependency-updates)), subject
to a minimum release age. When bumping `katex`, note that the pinned assets
style markup generated by Hugo's embedded KaTeX engine: check the KaTeX version
that the [transform.ToMath docs](https://gohugo.io/functions/transform/tomath/)
pair with the current Hugo, and verify a math-bearing page renders cleanly. A
`redoc` bump to 3.x needs more than the version line: Redoc 3 moves the CDN
script from `bundles/redoc.standalone.js` to `bundle/redoc.js`, so when Renovate
proposes 3.x, the `redoc` shortcode's URL change is part of that review.
