...
- `params.`_`PACKAGE`_`.version` for `mermaid`, `katex`, and `redoc`, read by
  the script partials and the `redoc` shortcode.
- The `version` field of the plugin's registry entry for `markmap`
  (`params.docsy.plugins.markmap.version`), read by the plugin loop and handed
  to the companion.
...
is enough. Guarded by the [script-version-pins test](#test-suites). The
templates' unset- and non-exact-version guards run reliably only on the docs,
blog, and swagger layouts for now: the default base template renders scripts
through an unkeyed `partialCached`, which can skip them. Before bumping, check
the [npm registry][npm-registry] and [OSV][] for advisories affecting the target
version. Verify that a page using the dependency (the diagrams and formulae
page, for example) renders with the new pin. Renovate proposes routine bumps
(see [Dependency updates](#dependency-updates)), subject to a minimum release
age. When bumping `katex`, note that the pinned assets style markup generated by
